...
class InformationStore {
    ...
    getBuriedPointType = () => {
        switch (this.editStatus) {
            case 'create':
                return 'new_information_mark';
            case 'modify':
                return 'modify_information_mark';
            default:
                break;
        }
    };
    ...
    isCreateMarker = () => {
        return DataLayerStore.editType === 'new_information_mark' || this.editStatus === 'create';
    };
    ...
    insertMarker = flow(function* (data) {
        try {
            const res = yield TaskService.tmsInfoErrorAdd(data);
            return res;
        } catch (e) {
            const msg = e.message || e || '';
            message.error('资料问题新增失败 ' + msg);
        }
    });
