...
export const testDataString = dataString => {
    const pattern = /^(((\[\((\Y\d{4})?\M\d{1,2}\)\{\M\d{1,2}\}(\&\(\Y\d{4}\)\{\Y\d{1,2}\})?(\&\(\Y\d{4}\M\d{1,2}\)\{\M\d{1,2}\})?(\]))?(\[\((W)?\D\d{1,2}\)\{\D\d{1,2}\})\]))?(\[\(\M\d{1,2}\D\d{1,2}\)\{\D\d{1,2}\}(\&\(\M\d{1,2}\)\{\M\d{1,2}\})?(\&\(\M\d{1,2}\D\d{1}\)\{\D\d{1,2}\})?\])?(\[\(h\d{1,2}m\d{1,2}\)\{h\d{1,2}(m\d{1,2})?\}\]&)*(\[\(h\d{1,2}m\d{1,2}\)\{h\d{1,2}(m\d{1,2})?\}\])?$/;
